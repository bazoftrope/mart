...
| `npm run db:backup` | Дамп БД (gzip) + ротация — `scripts/db-backup.sh` |
| `npm run db:backups` | Список имеющихся дампов |
| `npm run db:restore` | Восстановить БД из свежего дампа (нужен `-- --yes`) |
| `npm run db:backup:install-cron` | Ежедневный бэкап в crontab (03:00) |
...
- `video-hosting-plan.md` — план по видео: Kinescope vs свой сервер, что мешает в коде, оценка работ и стоимости, открытые вопросы.
